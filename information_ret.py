...
def read_data_and_create_vocab(whatToKnow, topic):
    ...
    if topic == 'eds_disease':
        if whatToKnow == 'symp':
            data = pd.read_csv("informationRetrivalBank/symp/edsSymp.csv")
        elif whatToKnow == 'types':
            data = pd.read_csv('informationRetrivalBank/types/edsTypes.csv')
...
# def main():
#     prompt = "what is eds symptoms?"
#     get_most_similar_docs(prompt)
